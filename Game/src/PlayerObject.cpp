...
PlayerObject::PlayerObject( PlayerInfo *player, Script *script, unsigned long type ) : AnimatedObject( script->GetStringData( "mesh_name" ), script->GetStringData( "mesh_path" ), type )
{
	...
	// Set the players's model.
	m_model = new char[strlen( player->model ) + 1];
	strcpy( m_model, player->model );
	...
	m_viewSmoothing = 0.7f;
	m_viewSensitivity = 0.6f;
	...
	m_weapons[0] = new Weapon( weaponScript, m_viewWeaponOffset, m_model );
	m_currentWeapon = 0;
	if( player->id == g_engine->GetNetwork()->GetID() )
	...
	m_playerAudioPath = new AudioPath3D;

	m_jumpHeaveSound = new Sound( "jump_uh.wav", g_engine->GetEngineSetup()->soundPath );
	m_deathSound = new Sound( "death_aarrrrhh.wav", g_engine->GetEngineSetup()->soundPath );
	m_deathBodySound = new Sound( "death_bodyhit.wav", g_engine->GetEngineSetup()->soundPath );

	//m_jumpHeaveSound->SetMinAttDist( 1.0f / g_engine->GetScale() );
	//m_deathSound->SetMinAttDist( 1.0f / g_engine->GetScale() );
	//m_deathBodySound->SetMinAttDist( 1.0f / g_engine->GetScale() );
...
PlayerObject::~PlayerObject()
{
	...
	SAFE_DELETE( m_leftStepAudioPath );
	SAFE_DELETE( m_rightStepAudioPath );

	SAFE_DELETE( m_playerAudioPath );

	SAFE_DELETE( m_jumpHeaveSound );
	SAFE_DELETE( m_deathSound );
	SAFE_DELETE( m_deathBodySound );
}
...
if( m_drive != 0.0f )
	Drive( m_drive * 6000.0f * elapsed, !GetFlyMode() );
if( m_strafe != 0.0f )
	Strafe( m_strafe * 5000.0f * elapsed, !GetFlyMode() );

	if( m_jumpInProgress == true && IsTouchingGround() == true )
	{
		m_jumpInProgress = false;
		SetFriction( m_origFriction );
	}

	if( m_jump == true && IsTouchingGround() == true )
	{
		m_playerAudioPath->Play( m_jumpHeaveSound->GetSound() );
		Jump( 3000.0f );
		m_jump = false;

		m_jumpInProgress = true;
		m_origFriction = m_surfacefriction;
		SetFriction( m_origFriction * 0.66f );
	}

	// Update the audio paths.
	D3DXVECTOR3 temp;

	temp = GetTranslation() + GetMesh()->GetReferencePoint( "rp_left_foot" )->GetTranslation();
	m_leftStepAudioPath->SetPosition( Vector3(temp.x, temp.y, temp.z) );
	temp = GetVelocity();
	m_leftStepAudioPath->SetVelocity( Vector3(temp.x, temp.y, temp.z) );

	temp = GetTranslation() + GetMesh()->GetReferencePoint( "rp_right_foot" )->GetTranslation();
	m_rightStepAudioPath->SetPosition( Vector3(temp.x, temp.y, temp.z) );
	temp = GetVelocity();
	m_rightStepAudioPath->SetVelocity( Vector3(temp.x, temp.y, temp.z) );

	m_playerAudioPath->SetPosition( Vector3(GetTranslation().x, GetTranslation().y, GetTranslation().z) );
	m_playerAudioPath->SetVelocity( Vector3(GetVelocity().x, GetVelocity().y, GetVelocity().z) );
	//
	
	// Check if the player is changing their weapon (local player only).
	static float move = 0.0f;
	if( m_id == g_engine->GetNetwork()->GetID() && m_changingWeapon > 0.0f )
	{
		m_changingWeapon -= elapsed;

		if( m_changingWeapon > 0.3f )
		{
		...
		else if( m_changingWeapon < 0.0f )
		{
			...
			pwcm.id = g_engine->GetNetwork()->GetID();
			pwcm.weapon = m_currentWeapon;
			g_engine->GetNetwork()->Send( &pwcm, sizeof( PlayerWeaponChangeMsg ) );
		}
		else
		{
	...
	else
	{
		...
		if( m_weaponChanging == false )
			m_weapons[m_currentWeapon]->Update( elapsed, m_fire, this, m_viewPoint );
	}
}
...
void PlayerObject::Render( D3DXMATRIX *world )
{
	...
	if( m_id != g_engine->GetNetwork()->GetID() )
...
void PlayerObject::CollisionOccurred( SceneObject *object, unsigned long collisionStamp )
{
	...
	if( ValidateSpawnerCollision(spawner) == false )
		return;
	...
	if( *spawner->GetObjectScript()->GetNumberData( "type" ) == WEAPON_SPAWN_OBJECT )
	{
		...
		if( m_weapons[listPosition] == NULL )
		{
			...
			m_weapons[listPosition] = new Weapon( spawner->GetObjectScript(), m_viewWeaponOffset, m_model );
			...
			{
				// Set the weapon to use the third person mesh.
	...
	else if( *spawner->GetObjectScript()->GetNumberData( "type" ) == AMMO_SPAWN_OBJECT )
	{
		char listPosition = (char)*spawner->GetObjectScript()->GetNumberData( "list_position" );
		GetWeapon(listPosition)->FillAmmo(GetWeapon(listPosition)->GetDefaultAmmo()/2);

		// If the local player's current weapon is empty, switch to this weapon. Other players will eventually get update about this player changing weapon
		if(m_id == g_engine->GetNetwork()->GetID() && GetCurrentWeaponIndex() != listPosition && GetCurrentWeapon()->IsLoaded() == false)
		{
			// Change to this weapon.
			ChangeWeapon( 0, listPosition );
		}
	}
	else if( *spawner->GetObjectScript()->GetNumberData( "type" ) == HEALTH_SPAWN_OBJECT )
	{
		float health = GetHealth() + 50.0f;
		if( health > 100.0f )
		{
			health = 100.0f;
		}
...
bool PlayerObject::ValidateSpawnerCollision(SpawnerObject *spawner)
{
	// Allow the base scene object to validate the collision.
	if(SceneObject::ValidateSpawnerCollision( spawner ) == false)
		return false;

	if( *spawner->GetObjectScript()->GetNumberData( "type" ) == AMMO_SPAWN_OBJECT )
	{
		char listPosition = (char)*spawner->GetObjectScript()->GetNumberData( "list_position" );

		if(GetWeapon(listPosition) == NULL || GetWeapon(listPosition)->GetValid() == false || GetWeapon(listPosition)->GetAmmo() == 999)
			return false;
	}
	else if( *spawner->GetObjectScript()->GetNumberData( "type" ) == HEALTH_SPAWN_OBJECT )
	{
	...
	else if( *spawner->GetObjectScript()->GetNumberData( "type" ) == WEAPON_SPAWN_OBJECT )
	{
		char listPosition = (char)*spawner->GetObjectScript()->GetNumberData( "list_position" );

		if(GetWeapon(listPosition) != NULL && GetWeapon(listPosition)->GetValid() == true)
			return false;
	}
...
void PlayerObject::Hurt( float damage, PlayerObject *attacker )
{
	...
	phm.id = m_id;
	phm.health = m_health - damage;
	phm.attackerid = attacker->GetID();
	g_engine->GetNetwork()->Send( &phm, sizeof( PlayerHealthMsg ) );
...
void PlayerObject::Kill()
{
	...
	m_playerAudioPath->Play( m_deathSound->GetSound() );
	PlayAnimation( ANIM_DEATH, 0.0f, false );
	m_playerAudioPath->Play( m_deathBodySound->GetSound() );
}

//-----------------------------------------------------------------------------
// Returns the player object's DirectPlay ID number.
//-----------------------------------------------------------------------------
unsigned int PlayerObject::GetID()
{
	return m_id;
}
...
//-----------------------------------------------------------------------------
// Returns the player object's model.
//-----------------------------------------------------------------------------
char *PlayerObject::GetModel()
{
	return m_model;
}
...
void PlayerObject::SetDrive( float drive )
{
	...
	if( drive == 0.0f )
	{
		if( m_drive != 0.0f && m_strafe == 0.0f /*&& m_jump == false*/ )
			PlayAnimation( ANIM_IDLE, 0.2f );
	}
	else if( m_drive == 0.0f && m_strafe == 0.0f /*&& m_jump == false*/ )
	{
		if( drive == 1.0f )
...
void PlayerObject::SetStrafe( float strafe )
{
	...
	if( strafe == 0.0f )
	{
		if( m_strafe != 0.0f && m_drive == 0.0f /*&& m_jump == false*/ )
			PlayAnimation( ANIM_IDLE, 0.2f );
	}
	else if( m_strafe == 0.0f && m_drive == 0.0f /*&& m_jump == false*/ )
	{
		if( strafe == 1.0f )
...
void PlayerObject::SetJump( bool jump )
{
	// Ignore if the player is dying (or dead)
	if( m_dying == true )
		return;
...
HRESULT CALLBACK PlayerObject::HandleCallback( THIS_ UINT Track, LPVOID pCallbackData )
{
	...
	if( data->foot == 1 )
	{
		...
		if( g_engine->GetSceneManager()->RayIntersectScene( &m_stepResult, GetTranslation() + GetMesh()->GetReferencePoint( "rp_left_foot" )->GetTranslation(), D3DXVECTOR3( 0.0f, -1.0f, 0.0f ) ) == true )
		{
			...
			{
				m_leftStepAudioPath->Play( ( (GameMaterial*)m_stepResult.material )->GetStepSound()->GetSound() );
			}
		}
	}
	else
	{
		...
		if( g_engine->GetSceneManager()->RayIntersectScene( &m_stepResult, GetTranslation() + GetMesh()->GetReferencePoint( "rp_right_foot" )->GetTranslation(), D3DXVECTOR3( 0.0f, -1.0f, 0.0f ) ) == true )
		{
			if( ( (GameMaterial*)m_stepResult.material )->GetStepSound() != NULL )
			{
				m_rightStepAudioPath->Play( ( (GameMaterial*)m_stepResult.material )->GetStepSound()->GetSound() );
			}
		}
	}
...
void PlayerObject::ChangeWeapon( char change, char weapon )
{
	...
	// If the player is already changing weapons, then ignore this request.
	...
	m_changingWeapon = 0.6f;

	// Send a weapon changing message.
	NetworkMessage pwcm;
	pwcm.msgid = MSGID_PLAYER_WEAPON_CHANGING;
	pwcm.id = g_engine->GetNetwork()->GetID();
	g_engine->GetNetwork()->Send( &pwcm, sizeof( NetworkMessage ) );
}
...
{
	// Reset all weapons' ammo
	for( unsigned char w = 0; w < 10; w++ )
		if( m_weapons[w] != NULL )
			m_weapons[w]->ResetAmmo();
...
char PlayerObject::GetCurrentWeaponIndex()
{
	return m_currentWeapon;
}

Weapon* PlayerObject::GetCurrentWeapon()
{
	return m_weapons[m_currentWeapon];
}

Weapon* PlayerObject::GetWeapon(char weapon)
{
	return m_weapons[weapon];
}