...
//-----------------------------------------------------------------------------
inline float GetLowestRoot( float a, float b, float c, float max, float *root )
{
	/*
	...
	*/
	...
	// sort so x1 <= x2
	if (r1 > r2) { float tmp=r2; r2=r1; r1=tmp; }
...
inline void CheckFace( CollisionData *data, D3DXVECTOR3 vertex0, D3DXVECTOR3 vertex1, D3DXVECTOR3 vertex2 )
{
	...
	if( angle > 0.0f )
		...
		// Ensure the object's (ellipsoid's) centre is in front of the polygon.
		// This was added so that the player does not get stuck at a place where the wall behind the current room's wall is also detected.
		if( D3DXPlaneDotCoord( &plane, &data->translation ) < -0.000001 )
			return;
	...
	if( fabsf(normalDotVelocity) <= 0.000001f)
	{
	...
	if( intersectFound == false )
	{
		...
		if( GetLowestRoot( a, b, c, intersectTime, &newTime ) )
		{
			intersectFound = true;
...
//-----------------------------------------------------------------------------
inline void CollideWithScene( CollisionData *data, Vertex *vertices, SceneFace *faces, unsigned long totalFaces, LinkedList< SceneObject > *objects, bool gravityIteration, unsigned long recursion = 5 )
{
	...
	D3DXVec3Normalize( &slidePlaneNormal, &( newTranslation - data->intersection ) );
		...
		if( gravityIteration == true )
		{
			// Dont let gravity act when the plane is 0 - 60 degrees inclined to the ground
			float alpha = (float)( asin( slidePlaneNormal.y ) * 180.0f / PI );
		
			if( (alpha >= (30.0f)) && (alpha <= (90.0f)) )
				return;
		}
	...
	recursion--;
	if( recursion > 0 )
		CollideWithScene( data, vertices, faces, totalFaces, objects, gravityIteration, recursion );
}
...
inline void PerformCollisionDetection( CollisionData *data, Vertex *vertices, SceneFace *faces, unsigned long totalFaces, LinkedList< SceneObject > *dynamicObjects )
{
	...
	CollideWithScene( data, vertices, faces, totalFaces, dynamicObjects, false );

	if( data->object->GetFlyMode() == false )
	{
		...
		CollideWithScene( data, vertices, faces, totalFaces, dynamicObjects, true );
	}
	...
	for( unsigned long f = 0; f < totalFaces; f++ )
	{
		...
		{
			if( distance < hitDistance || hitDistance == limit )
