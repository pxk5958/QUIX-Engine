...
SceneManager::SceneManager( float scale, char* spawnerPath )
{
	...
	m_firstOctreeNode = NULL;

	m_maxOctreeFaces = 0;
	m_maxOctreeHalfSize = 0.0f;
}
...
void SceneManager::LoadScene( char* name, char* path )
{
	...
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_LIGHTING, true );
	g_engine->GetRenderer()->GetDevice()->SetLight( 0, &sun );
	g_engine->GetRenderer()->GetDevice()->LightEnable( 0, true );
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_SPECULARENABLE, true );

//is this necessary? if so, here or setuprenderstates?
g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_AMBIENT, 0x000000 );


	// Set up the fog.
	float density = *script->GetFloatData( "fog_density" ) * m_scale;
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_FOGENABLE, true );
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_FOGCOLOR, D3DCOLOR_COLORVALUE( script->GetColourData( "fog_colour" )->r, script->GetColourData( "fog_colour" )->g, script->GetColourData( "fog_colour" )->b, script->GetColourData( "fog_colour" )->a ) );
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_FOGVERTEXMODE, D3DFOG_EXP2 );
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_FOGDENSITY, *(unsigned long*)&density );


	// Store the constraints used for creating the scene.

	m_maxOctreeFaces = *script->GetNumberData( "max_octree_faces" );
	m_maxOctreeHalfSize = *script->GetFloatData( "max_octree_half_size" );
	...
	D3DXMatrixPerspectiveFovLH( &projection, D3DXToRadian( g_engine->GetFov() ), (float)display->Width / (float)display->Height, 0.01f / m_scale, m_mesh->GetBoundingSphere()->radius * 2.0f );
	...
	for( unsigned long m = 0; m < m_mesh->GetStaticMesh()->NumMaterials; m++ )
	{
		...
		if( found == false && m_mesh->GetStaticMesh()->materials[m]->GetIgnoreFace() == false )
			m_renderCaches->Add( new RenderCache( g_engine->GetRenderer()->GetDevice(), m_mesh->GetStaticMesh()->materials[m] ) );
	}
	...
	while( frames->Iterate() != NULL )
	{
		...
		if( strncmp( "sp_", frames->GetCurrent()->Name, 3 ) == 0 )
		{
			...
			else
			{
				...
				SpawnerObject *spawner = NULL;


					try
					{
						spawner = new SpawnerObject( name, m_spawnerPath );
					}
					catch( std::bad_alloc& )
					{
						SAFE_DELETE( spawner );
...
//}
	...
	if( NeedDefaultMaterial == true )
		m_renderCaches->Add( new RenderCache( g_engine->GetRenderer()->GetDevice(), g_engine->GetMaterialManager()->Add( "defaultMaterial.dds.txt" ) ) );

	// Create the array of faces.
	//m_faces = new SceneFace[m_totalFaces];
	m_faces.resize(m_totalFaces);
	...
	for( unsigned long f = 0; f < m_mesh->GetStaticMesh()->originalMesh->GetNumFaces(); f++ )
	{
		...
		m_tempVertices[m_faces[f].vertex0] = vertices[m_faces[f].vertex0];
		m_tempVertices[m_faces[f].vertex1] = vertices[m_faces[f].vertex1];
		m_tempVertices[m_faces[f].vertex2] = vertices[m_faces[f].vertex2];
	}
	...
	// Build the OCTREE
	m_firstOctreeNode = new SceneOCTREENode();
	RecursiveOctreeSceneBuild( m_firstOctreeNode, m_mesh->GetBoundingSphere()->centre, m_mesh->GetBoundingBox()->halfSize );
	

	// Create the vertex buffer that will hold all the vertices in the scene.
	g_engine->GetRenderer()->GetDevice()->CreateVertexBuffer( m_totalVertices * VERTEX_FVF_SIZE, D3DUSAGE_WRITEONLY, VERTEX_FVF, D3DPOOL_MANAGED, &m_sceneVertexBuffer, NULL );
	...
	memcpy( m_vertices, &m_tempVertices[0], m_tempVertices.size() * VERTEX_FVF_SIZE );
	...
	std::vector<Vertex>().swap(m_tempVertices);
...
void SceneManager::DestroyScene()
{
	...
	std::vector<SceneFace>().swap(m_faces);
	m_totalFaces = 0;
	...
	// Destroy the scene hierarchy.
	SAFE_DELETE( m_firstOctreeNode );
...
void SceneManager::Update( float elapsed, D3DXMATRIX *view )
{
	...
	while( m_dynamicObjects->Iterate() )
	{
		...
		m_totalCollisionFaces = 0;
		RecursiveBuildCollisionArray( m_firstOctreeNode, m_dynamicObjects->GetCurrent() );
...
//-----------------------------------------------------------------------------
void SceneManager::Render( D3DXVECTOR3 viewer )
{
	// Ensure a scene is loaded.
	if( m_firstOctreeNode == NULL )
		return;
	...
	RecursiveSceneFrustumCheck( m_firstOctreeNode, viewer, true );
	...
	RecursiveSceneOcclusionCheck( m_firstOctreeNode );


	// Set an identity world transformation matrix to render around the origin.
	D3DXMATRIX world;
	D3DXMatrixIdentity( &world );
	g_engine->GetRenderer()->GetDevice()->SetTransform( D3DTS_WORLD, &world );

	// Set the scene vertex buffer as the current device data stream.
	g_engine->GetRenderer()->GetDevice()->SetStreamSource( 0, m_sceneVertexBuffer, 0, VERTEX_FVF_SIZE );
	g_engine->GetRenderer()->GetDevice()->SetFVF( VERTEX_FVF );
	...
	while( m_dynamicObjects->Iterate() )
	{
		...
		if( m_viewFrustum.IsSphereInside( m_dynamicObjects->GetCurrent()->GetBoundingSphere()->centre, m_dynamicObjects->GetCurrent()->GetBoundingSphere()->radius ) < 0 )
			continue;
...
bool SceneManager::RayIntersectScene( RayIntersectionResult *result, D3DXVECTOR3 rayPosition, D3DXVECTOR3 rayDirection, bool checkScene, SceneObject *thisObject, bool checkObjects )
{
	...
	if( checkScene == true )
		RecursiveSceneRayCheck( m_firstOctreeNode, result, rayPosition, rayDirection, &hitDistance );
...
//-----------------------------------------------------------------------------
void SceneManager::RecursiveSceneFrustumCheck( SceneOCTREENode *node, D3DXVECTOR3 viewer, bool testIntersection )
{
	if( testIntersection == true )
	{
		if( IsPointInBox( viewer, node->GetBoundingBox()->min, node->GetBoundingBox()->max ) == false )
		{
			// Check if the node's bounding sphere is inside the view frustum.
			int isSphereInside = m_viewFrustum.IsSphereInside( node->GetBoundingSphere()->centre, node->GetBoundingSphere()->radius );
			if( isSphereInside < 0 )
				return;

			// Check if the node's bounding box is inside the view frustum.
			int isBoxInside = m_viewFrustum.IsBoxInside( node->GetBoundingBox()->min, node->GetBoundingBox()->max );
			if( isBoxInside < 0 )
				return;
			else if( isBoxInside == 1 )
				testIntersection = false;
		}
	}


	// Set the visible stamp on this node to the current frame stamp. This will
	// indicate that the node may be visible this frame and may need rendering.
	node->visibleStamp = m_frameStamp;


	// Check if any of this node's children are visible.
	if( node->isLeaf == false )
	{
		for( char c = 0; c < 8; c++ )
		if( node->children[c] != NULL )
			RecursiveSceneFrustumCheck( node->children[c], viewer, testIntersection );
	}
	else
	{

		// Iterate through all the occluders in this leaf.
		node->occluders->Iterate( true );
		while( node->occluders->Iterate() )
		{
			// Check if the occluder's bounding sphere is inside the view frustum.
			if( m_viewFrustum.IsSphereInside( node->occluders->GetCurrent()->translation, node->occluders->GetCurrent()->GetBoundingSphere()->radius ) < 0 )
				continue;
	
			// Check if the occluder's bounding box is inside the view frustum.
			if( m_viewFrustum.IsBoxInside( node->occluders->GetCurrent()->GetBoundingBox()->min, node->occluders->GetCurrent()->GetBoundingBox()->max ) < 0 )
				continue;

			// Calculate the distance between the occluder and the viewer.
			node->occluders->GetCurrent()->distance = D3DXVec3Length( &( node->occluders->GetCurrent()->translation - viewer ) );
			...
			while( m_visibleOccluders->Iterate() )
			{
				...
				if( node->occluders->GetCurrent() == m_visibleOccluders->GetCurrent() )
					break;

				// If the new occluder is closer to the viewer than this occluder,
				// then add it to the list before this occluder.
				if( node->occluders->GetCurrent()->distance < m_visibleOccluders->GetCurrent()->distance )
				{
					m_visibleOccluders->InsertBefore( node->occluders->GetCurrent(), m_visibleOccluders->GetCompleteElement( m_visibleOccluders->GetCurrent() ) );
					node->occluders->GetCurrent()->visibleStamp = m_frameStamp;
					break;
				}
			}

			// If the occluder wasn't in the list or not added then add it now.
			if( node->occluders->GetCurrent()->visibleStamp != m_frameStamp )
			{
				m_visibleOccluders->Add( node->occluders->GetCurrent() );
				node->occluders->GetCurrent()->visibleStamp = m_frameStamp;
			}
		}
...
//-----------------------------------------------------------------------------
void SceneManager::RecursiveSceneOcclusionCheck( SceneOCTREENode *node )
{
	// Ignore the node if it is not visible this frame.
	if( node->visibleStamp != m_frameStamp )
		return;	
	...
	while( m_visibleOccluders->Iterate() )
	{
		...
		// If the node's bounding sphere is overlapping the occluder's volume
		// and the leaf's bounding box is completely enclosed by the occluder's
		// volume, then the leaf is hidden, so ignore it.
		if( IsSphereOverlappingVolume( m_visibleOccluders->GetCurrent()->planes, node->GetBoundingSphere()->centre, node->GetBoundingSphere()->radius ) == true )
			if( IsBoxEnclosedByVolume( m_visibleOccluders->GetCurrent()->planes, node->GetBoundingBox()->min, node->GetBoundingBox()->max ) == true )
				return;
	}
	...
	if(node->isLeaf == false)
	{
		for( char c = 0; c < 8; c++ )
		if( node->children[c] != NULL )
			RecursiveSceneOcclusionCheck( node->children[c] );
	}
	else
	{
			
		// Go through all the faces in the leaf.
		for( unsigned long f = 0; f < node->totalFaces; f++ )
		{
			...
			if( m_faces[node->faces[f]].renderStamp == m_frameStamp )
				continue;

			// Set the face's render stamp to indicate that it has been rendered.
			m_faces[node->faces[f]].renderStamp = m_frameStamp;

			// Tell the face's render cache to render this face.
			m_faces[node->faces[f]].renderCache->RenderFace( m_faces[node->faces[f]].vertex0, m_faces[node->faces[f]].vertex1, m_faces[node->faces[f]].vertex2 );
		}
...
//----------------------------------------------------------------------------- 
void SceneManager::RecursiveSceneRayCheck( SceneOCTREENode *node, RayIntersectionResult *result, D3DXVECTOR3 rayPosition, D3DXVECTOR3 rayDirection, float *hitDistance )
{
    // Check the ray against the scene node.
    if( D3DXBoxBoundProbe( &node->GetBoundingBox()->min, &node->GetBoundingBox()->max, &rayPosition, &rayDirection ) == false ) 
        return;
    ...
    if(node->isLeaf == false)
	{
		...
		for( char c = 0; c < 8; c++ )
			if( node->children[c] != NULL )
				RecursiveSceneRayCheck( node->children[c], result, rayPosition, rayDirection, hitDistance );
	}
	else
	{
		...
		for( unsigned long f = 0; f < node->totalFaces; f++ )
		{
			...
			if( m_faces[node->faces[f]].renderCache->GetMaterial()->GetIgnoreRay() == true ) 
				continue;

			// Check the ray against this face.
			if( D3DXIntersectTri( (D3DXVECTOR3*)&m_vertices[m_faces[node->faces[f]].vertex0], (D3DXVECTOR3*)&m_vertices[m_faces[node->faces[f]].vertex1], (D3DXVECTOR3*)&m_vertices[m_faces[node->faces[f]].vertex2], &rayPosition, &rayDirection, NULL, NULL, hitDistance ) == TRUE ) 
			{
				if( *hitDistance < result->distance || result->material == NULL )
				{
					( *result ).distance = *hitDistance;
					( *result ).material = m_faces[node->faces[f]].renderCache->GetMaterial(); 
				}
			}
		}
	}
...
//-----------------------------------------------------------------------------
void SceneManager::RecursiveBuildCollisionArray( SceneOCTREENode *node, SceneObject *object )
{
	// Only process the node if the object's bounding box intesects with it.
	// NOTE: Test both ways to ensure smaller nodes are accepted for intersection.
	if( !IsBoxInBox( object->GetBoundingBox()->min, object->GetBoundingBox()->max, node->GetBoundingBox()->min, node->GetBoundingBox()->max ) && !IsBoxInBox( node->GetBoundingBox()->min, node->GetBoundingBox()->max, object->GetBoundingBox()->min, object->GetBoundingBox()->max ) )
		return;

	if(node->isLeaf == false)
	{
		for( int i = 0; i < 8; i++ )
			//Eventhough the parent is not a leaf, some of the children may not be valid.
			if( node->children[i] != NULL )
				RecursiveBuildCollisionArray( node->children[i], object );
	}
	else
	{
		...
		for( unsigned long f = 0; f < node->totalFaces; f++ )
		{
			...
			m_collisionFaces[m_totalCollisionFaces] = m_faces[node->faces[f]];
			m_totalCollisionFaces++;
		}
	}
}

void SceneManager::RecursiveOctreeSceneBuild( SceneOCTREENode *leaf, D3DXVECTOR3 translation, float halfSize )
{
	// Build a bounding volume around this node.
	...
	// Count the number of face in this node.
	unsigned long totalFaces = 0;
	for( unsigned long f = 0; f < m_totalFaces; f++ )
		if( IsFaceInBox( &m_tempVertices[m_faces[f].vertex0], &m_tempVertices[m_faces[f].vertex1], &m_tempVertices[m_faces[f].vertex2], leaf->GetBoundingBox()->min, leaf->GetBoundingBox()->max ) == true )
			totalFaces++;

	// Only divide the node up if it is too big and contains too many faces.
	if( halfSize > m_maxOctreeHalfSize && totalFaces > m_maxOctreeFaces )
	{
		...
		for( char c = 0; c < 8; c++ )
		{
			...
			// Calculate the bounding box around the node leaf.
			...
			// Check if the new scene node will have at least one face in it.
			for( unsigned long f = 0; f < m_totalFaces; f++ )
			{
				if( IsFaceInBox( &m_tempVertices[m_faces[f].vertex0], &m_tempVertices[m_faces[f].vertex1], &m_tempVertices[m_faces[f].vertex2], newMin, newMax ) == true )
				{
					leaf->isLeaf = false;

					// A face has been found that is inside the new child scene
					// node, so create the scene node. Then recurse through the
					// scene node's branch of the scene hierarchy.
					leaf->children[c] = new SceneOCTREENode();
					RecursiveOctreeSceneBuild( leaf->children[c], newTranslation, halfSize / 2.0f );
	...
	// If no single face was found in a child node, then the parent is still a leaf. Check if it is so and add faces to it.
	if( leaf->isLeaf == true )
	{
