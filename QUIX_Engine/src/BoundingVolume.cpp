...
//Assumes centre is 0. So it should be set before any translation
void BoundingVolume::SetBoundingBox( D3DXVECTOR3 centre, float radius )
{
	m_box->halfSize = radius;

	m_originalMin = m_box->min = D3DXVECTOR3( centre.x - radius, centre.y - radius, centre.z - radius ); 
	m_originalMax = m_box->max = D3DXVECTOR3( centre.x + radius, centre.y + radius, centre.z + radius );
}
