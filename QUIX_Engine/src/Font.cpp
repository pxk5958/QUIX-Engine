...
Font::Font( char*  name, short size, unsigned long bold, bool italic )
{
	...
	if( FAILED( g_engine->GetRenderer()->GetDevice()->CreateTexture( m_textureWidth, m_textureHeight, 1, 0, D3DFMT_A4R4G4B4, D3DPOOL_MANAGED, &m_texture, NULL ) ) )
		goto End;
	...
	g_engine->GetRenderer()->GetDevice()->CreateVertexBuffer( 1020 * sizeof( TLVertex ), D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, 0, D3DPOOL_DEFAULT, &m_vb, NULL );

	// Prepare the alpha testing for rendering the characters.
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_ALPHAREF, 0x08 );
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_ALPHAFUNC, D3DCMP_GREATEREQUAL );

	// Create a state block for capturing the volatile render states.
	g_engine->GetRenderer()->GetDevice()->BeginStateBlock();
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_LIGHTING, false );
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_ALPHATESTENABLE, false );
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_FOGENABLE, false );
	g_engine->GetRenderer()->GetDevice()->EndStateBlock( &m_states );
...
void Font::Render( char* text, float x, float y, D3DCOLOR colour )
{
	...
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_LIGHTING, false );
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_ALPHATESTENABLE, true );
	g_engine->GetRenderer()->GetDevice()->SetRenderState( D3DRS_FOGENABLE, false );

	// Set the non-volatile render states.
	g_engine->GetRenderer()->GetDevice()->SetTexture( 0, m_texture );
	g_engine->GetRenderer()->GetDevice()->SetFVF( TL_VERTEX_FVF );
	g_engine->GetRenderer()->GetDevice()->SetStreamSource( 0, m_vb, 0, TL_VERTEX_FVF_SIZE );
		...
		if( c != ' ' )
		{
			...
			*vertices++ = TLVertex( D3DXVECTOR4( x - 0.5f, y - 0.5f, 0.0f, 1.0f ), colour, tx1, ty1 );
			...
			if( totalTriangles == 340 )
			{
				...
				m_vb->Unlock();
				g_engine->GetRenderer()->GetDevice()->DrawPrimitive( D3DPT_TRIANGLELIST, 0, totalTriangles );
				vertices = NULL;
				m_vb->Lock( 0, 0, (void**)&vertices, D3DLOCK_DISCARD );
				totalTriangles = 0;
			}
