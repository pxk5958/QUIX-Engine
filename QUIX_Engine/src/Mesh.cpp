...
/*D3DXMatrixIdentity(&frame->finalTransformationMatrix);
...
HRESULT AllocateHierarchy::CreateMeshContainer( THIS_ LPCSTR Name, CONST D3DXMESHDATA *pMeshData, CONST D3DXMATERIAL *pMaterials, CONST D3DXEFFECTINSTANCE *pEffectInstances, DWORD NumMaterials, CONST DWORD *pAdjacency, LPD3DXSKININFO pSkinInfo, LPD3DXMESHCONTAINER *ppNewMeshContainer )
{
	...
	if( ( meshContainer->NumMaterials = NumMaterials ) > 0 )
	{
		...
		for( unsigned long m = 0; m < NumMaterials; m++ )
		{
			if( pMaterials[m].pTextureFilename )
			{
				...
				strcpy( meshContainer->materialNames[m], pMaterials[m].pTextureFilename );
			}
			else
	...
	if( pSkinInfo != NULL )
	{
		...
		meshContainer->originalMesh->CloneMeshFVF( D3DXMESH_MANAGED, meshContainer->originalMesh->GetFVF(), g_engine->GetRenderer()->GetDevice(), &meshContainer->MeshData.pMesh );
...
AllocateHierarchy *ah = new AllocateHierarchy;

	D3DXLoadMeshHierarchyFromXA( GetFilename(), D3DXMESH_MANAGED, g_engine->GetRenderer()->GetDevice(), ah, NULL, (D3DXFRAME**)&m_firstFrame, &m_animationController );
...
SAFE_DELETE( ah );
	...
	D3DXLoadMeshFromXA( GetFilename(), D3DXMESH_MANAGED, g_engine->GetRenderer()->GetDevice(), &adjacencyBuffer, &materialBuffer, NULL, &m_staticMesh->NumMaterials, &m_staticMesh->originalMesh );
...
void Mesh::RenderFrame( Frame *frame )
{
	...
	if( frame->pMeshContainer != NULL )
	{
		...
		if( meshContainer->pSkinInfo != NULL )
		{
			...
			{
				g_engine->GetRenderer()->GetDevice()->SetMaterial( meshContainer->materials[meshContainer->attributeTable[a].AttribId]->GetLighting() );
				g_engine->GetRenderer()->GetDevice()->SetTexture( 0, meshContainer->materials[meshContainer->attributeTable[a].AttribId]->GetTexture() );
		...
		else
		{
			...
			for( unsigned long m = 0; m < meshContainer->NumMaterials; m++)
			{
				...
				{
					g_engine->GetRenderer()->GetDevice()->SetMaterial( meshContainer->materials[m]->GetLighting() );
					g_engine->GetRenderer()->GetDevice()->SetTexture( 0, meshContainer->materials[m]->GetTexture() );
				}
				else
					g_engine->GetRenderer()->GetDevice()->SetTexture( 0, NULL );
