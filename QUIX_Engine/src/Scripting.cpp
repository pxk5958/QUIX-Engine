...
Variable::Variable( char* name, FILE *file )
{
	...
	else if( strcmp( buffer, "colour" ) == 0 )
	{
		...
		fscanf( file, "%s", buffer );
		colour.r = (float)strtod( buffer, 0);
		fscanf( file, "%s", buffer );
		colour.g = (float)strtod( buffer, 0);
		fscanf( file, "%s", buffer );
		colour.b = (float)strtod( buffer, 0);
		fscanf( file, "%s", buffer );
		colour.a = (float)strtod( buffer, 0);
	...
	else if( strcmp( buffer, "vector" ) == 0 )
	{
		...
		D3DXVECTOR3 vector;
		fscanf( file, "%s", buffer );
		vector.x = (float)strtod( buffer, 0 );
		fscanf( file, "%s", buffer );
		vector.y = (float)strtod( buffer, 0 );
		fscanf( file, "%s", buffer );
		vector.z = (float)strtod( buffer, 0 );
...
{
	FILE *file = NULL;
	char output[MAX_PATH*sizeof(char)];
