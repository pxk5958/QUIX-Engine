...
SceneObject::SceneObject( unsigned long type, char* meshName, char* meshPath, bool sharedMesh )
{
	...
	//m_gravity = g_engine->GetSceneManager()->GetGravity();
	m_flyMode = false;
/*
	m_appliedforce = D3DXVECTOR3( 0.0f, 0.0f, 0.0f );
    m_traction = 1.0f;
...
float friction = 1.0f - m_surfacefriction * elapsed;
...
*/
    ...
    fSpeed = D3DXVec3Length( &m_velocity );

    // Calculate drag / air resistance (relative to the speed squared).
    vecDrag = -m_airresistance * (m_velocity * fSpeed);
    
    // Calculate the friction force
    vecFriction = -(m_traction * m_surfacefriction) * m_velocity;
...
void SceneObject::Render( D3DXMATRIX *world )
{
	...
	if( world == NULL )
		g_engine->GetRenderer()->GetDevice()->SetTransform( D3DTS_WORLD, &m_worldMatrix );
	else
...
bool SceneObject::ValidateSpawnerCollision(SpawnerObject *spawner)
{
	...
	if( GetType() == TYPE_SCENE_OBJECT || GetType() == TYPE_ANIMATED_OBJECT || GetType() == TYPE_SPAWNER_OBJECT )
		return false;
...
if( lockYAxis == false )
	...
	//m_appliedforce += ( m_forward * force / g_engine->GetScale() );
}
...
if( lockYAxis == false )
	...
	//m_appliedforce += ( m_right * force / g_engine->GetScale() );
}


void SceneObject::Jump( float force/*, bool lockXZAxis*/ )
{

/*
D3DXVECTOR3 realForce = m_up * force;
...
void SceneObject::ForceAlongDir( float force, D3DXVECTOR3 normalizedDirection )
{

normalizedDirection *= force;

m_velocity.y += normalizedDirection.y;
m_velocity.x += normalizedDirection.x;
m_velocity.z += normalizedDirection.z;
...
D3DXVECTOR3 SceneObject:: GetUpVector()
{
	return m_up;
}
